...
public class MainActivity extends AppCompatActivity {
    ...
    public void getWeather(View v) {
        Double lat = 42.8700000;
        final Double lng = 74.5900000;
        String units = "metric";
        ...
        Call<ModelWeatherDay> callToday = api.getToday(lat, lng, units, key);
        callToday.enqueue(new Callback<ModelWeatherDay>() {
            @Override
            public void onResponse(Call<ModelWeatherDay> call, Response<ModelWeatherDay> response) {
                Log.e(TAG, "onResponse");
                ModelWeatherDay data = response.body();
            ...
            @Override
            public void onFailure(Call<ModelWeatherDay> call, Throwable t) {
                Log.e(TAG, "onFailure");
        ...
        Call<ModelWeatherForecast> callForecast = api.getForecast(lat, lng, units, key);
        callForecast.enqueue(new Callback<ModelWeatherForecast>() {
            @Override
            public void onResponse(Call<ModelWeatherForecast> call, Response<ModelWeatherForecast> response) {
                Log.e(TAG, "onResponse");
                ModelWeatherForecast data = response.body();
                //Log.d(TAG,response.toString());
                ...
                if (response.isSuccessful()) {
                    ...
                    for (ModelWeatherDay day : data.getItems()) {
            ...
            @Override
            public void onFailure(Call<ModelWeatherForecast> call, Throwable t) {
                Log.e(TAG, "onFailure");
                Log.e(TAG, t.toString());
            }
        });
